class aes:
    ...
    def aes_decrypt(self, ciphertext_list, RoundKeys):
        ...
        State = self.AddRoundKey(State, RoundKeys, 0)
        return State
def AES(en_or_de,text,key):
    if en_or_de==0:
        aesen=aes()
        return hex(aesen._16bytes2num(aesen.aes_encrypt(aesen.num_2_16bytes(text),aesen.round_key_generator(key))))
    else:
        aesde=aes()       
        return hex(aesde._16bytes2num(aesde.aes_decrypt(aesde.num_2_16bytes(text),aesde.round_key_generator(key))))   
